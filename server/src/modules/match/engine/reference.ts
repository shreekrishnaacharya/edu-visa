// ---------------------------------------------------------------------------
// Normalization / reference functions — ported VERBATIM from the prototype's
// src/mocks/db/reference.ts.  In this backend the *rates and bands* come from
// the DB-backed `reference` module (ReferenceService), but the shape of the
// conversion is frozen here so the golden test can pin it.
// ---------------------------------------------------------------------------
...
export function toCanonicalGpa(value: number, scale: GpaScale): number {
  switch (scale) {
    ...
    case 'percentage':
      return clamp(value);
    case 'division':
      // 1st division ≈ 68, 2nd ≈ 52, 3rd ≈ 40 (Nepal/India board convention)
